...
public class CustomExplosion {
    ...
    public void explodeExcluding(@Nullable Entity entityIn, int radiusTwo, SoundEvent sound) {
        Random rand = new Random();
        int a = this.radius;
        int b = radiusTwo;
        for(int x = -a; x <= a; x++) {
        ...
        world.playSound((PlayerEntity)null, pos.getX(), pos.getY(), pos.getZ(), sound, SoundCategory.BLOCKS, 0.8F, -0.4F);
        playerKnockbackMap.forEach((player, motion) -> player.setMotion(player.getMotion().add(motion.scale(1.75F))));
        for(int i = 0; i < set.size(); i++) {
            world.setBlockState(set.get(i), Blocks.AIR.getDefaultState());
            Vec3d motionParticle = new Vec3d(set.get(i).getX()-pos.getX(), set.get(i).getY()-pos.getY(), set.get(i).getZ()-pos.getZ()).scale(0.25D);
    ...
    public void explodeExcluding(@Nullable Entity entityIn, SoundEvent sound) {
        Random rand = new Random();
        int a = this.radius;
        int b = (int) (this.radius*this.distFactor);
        for(int x = -a; x <= a; x++) {
